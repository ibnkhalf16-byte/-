...
class AccountingEngine {
  static String normalizeItem(String item) =>
      item.trim().toLowerCase().replaceAll(RegExp(r'\s+'), ' ');
  ...
  /// حساب تكلفة البضاعة المباعة (COGS) والربح الحقيقي
  static Map<String, dynamic> calculateRealProfits(List<TripModel> rows) {
    ...
    for (var p in rows.where((t) => t.operation == 'purchase')) {
      String it = normalizeItem(p.item);
    ...
    for (var s in rows.where((t) => t.operation == 'sale')) {
      ...
      String it = normalizeItem(s.item);
  ...
  /// كشف الحساب التراكمي مرتباً زمنياً مع احتساب الرصيد بعد كل حركة
